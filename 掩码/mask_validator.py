...
def calculate_fidelity(edge_mask_tensor: torch.Tensor, threshold: float) -> float:
    ...
    # 保真度：关键边比例越高，掩码区分度越高，保真度越高
    fidelity = key_edge_ratio * (1 + mean_importance) * (1 + std_importance)
...
def calculate_explanation_loss(edge_mask_tensor: torch.Tensor, lambda1: float = 0.1, lambda2: float = 0.1) -> float:
    """
    计算解释损失函数
    ℒ_exp = (Ŷ - Ŷ_M)^2 + λ₁Σ∥M_k∥₁ + λ₂Σ∥M_k - M_{k-1}∥₂²
    ...
    # 1. 保真度损失：这里简化为掩码的平均重要性（实际应该用预测差异）
    fidelity_loss = (1.0 - edge_mask_tensor.mean().item()) ** 2
    
    # 2. 稀疏性损失：L1正则化
    sparsity_loss = 0.0
    for t in range(num_time_steps):
        sparsity_loss += torch.norm(edge_mask_tensor[t], p=1).item()
    sparsity_loss *= lambda1
    
    # 3. 平滑性损失：相邻时间片差异的L2正则化
    smoothness_loss = 0.0
    for t in range(1, num_time_steps):
        smoothness_loss += torch.norm(edge_mask_tensor[t] - edge_mask_tensor[t-1], p=2).item() ** 2
    smoothness_loss *= lambda2
...
def analyze_dataset(dataset_name: str, thresholds: List[float] = [0.3, 0.5, 0.7]) -> Dict[str, Any]:
    ...
    for threshold in thresholds:
        ...
        # 计算解释损失
        loss = calculate_explanation_loss(edge_mask_tensor)
